...
class MainActivity : AppCompatActivity() {
    ...
    private fun setListener() {
        ...
        findViewById<Button>(R.id.btnInsert).setOnClickListener {
            // 判斷是否有填入書名或價格
            if (edBook.length() < 1 || edPrice.length() < 1 || edYear.length() < 1)
                showToast("欄位請勿留空")
            else
                try {
                    // 新增一筆書籍紀錄於myTable資料表
                    ...
                    showToast("新增:${edBook.text},價格:${edPrice.text},年份:${edYear.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("新增失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btnUpdate).setOnClickListener {
            ...
            if (edBook.length() < 1 || edPrice.length() < 1 || edYear.length() < 1)
                ...
                try {
                    ...
                    showToast("更新:${edBook.text},價格:${edPrice.text},價格:${edYear.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("更新失敗:$e")
                }
